...
public class BookmarkButton extends ToggleButtonComponent {
	...
	public void drawIcon(GuiGraphicsExtractor context, int x, int y) {
		context.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, iconTexture, x, y, 0f, 0f, iconWidth, iconHeight, iconWidth, iconHeight);
	}
	...
	public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float partialTick) {
		...
		context.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, backgroundTexture, getGuiX(), getGuiY(), !vertical || isExtended ? 0f : HEIGHT, vertical || isExtended ? 0f : HEIGHT, vertical ? HEIGHT : WIDTH, vertical ? WIDTH : HEIGHT, vertical ? HEIGHT * 2 : WIDTH, vertical ? WIDTH : HEIGHT * 2);
	...
	public void renderTooltip(GuiGraphicsExtractor context, int mouseX, int mouseY, float partialTick, boolean mouseOver) {
		if (mouseOver && !title.getString().isEmpty()) {
			context.setTooltipForNextFrame(title, mouseX, mouseY);
		}
	}
}
